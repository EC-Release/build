...
def main():
    ...
    os.system("CGO_ENABLED=0 GOOS=linux GODEBUG=netdns=cgo GOARCH=amd64 go build -v -o /{}_linux_sys {}/*.go".format(BINARY,DHOME))
    
    print "generate linux_amd64 artifacts with race; dns resolved by go"
    os.system("CGO_ENABLED=0 GOOS=linux GODEBUG=netdns=go GOARCH=amd64 go build -v -o /{}_linux_var {}/*.go".format(BINARY,DHOME))
